...
pub(crate) fn topic_index_prefix(topic_prefix: &str) -> String {
    format!("{PREFIX}topic/{topic_prefix}/streams/")
}

/// Key registering `stream_key` under a topic index.
pub(crate) fn topic_stream_key(topic_prefix: &str, stream_key: &str) -> Key {
    Key::from(format!(
        "{}{stream_key}",
...
pub(crate) fn scan_end(prefix: &str) -> Key {
...
pub(crate) fn seq_from_event_key(key: &[u8]) -> Result<Seq> {
...
pub(crate) fn stream_key_from_topic_index(key: &[u8], topic_prefix: &str) -> Result<String> {
    let text = std::str::from_utf8(key).map_err(|e| LogError::Backend(e.to_string()))?;
