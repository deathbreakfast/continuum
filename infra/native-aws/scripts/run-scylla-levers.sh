#!/usr/bin/env bash
# Master driver: Scylla optimization levers Tracks Z1–Z5 + X/Y re-runs (WSL-resilient).
# Usage: run-scylla-levers.sh [--no-teardown-on-fail] [--skip-artifact] [--from-step STEP]
set -euo pipefail
...
STATE_FILE="$STATE_DIR/scylla-levers.json"
...
# Phase 1: colocated Z2, Z3, Y re-run
run_step colocated_provision_1 provision_colocated
run_step Z2_colocated bash "$ROOT/scripts/run-scylla-track-z.sh" native-colocated z2 256
run_step Z3_colocated bash "$ROOT/scripts/run-scylla-track-z.sh" native-colocated z3 256
run_step Y_colocated bash "$ROOT/scripts/run-scylla-track-y.sh" native-colocated
run_step fetch_colocated_1 bash "$ROOT/scripts/fetch-reports.sh" native-colocated --upload-s3 || true
bash "$ROOT/scripts/upload-diagnosis-state.sh" || true
run_step teardown_colocated_1 bash "$ROOT/scripts/teardown.sh" native-colocated

# Phase 2: scylla-2n X re-run
...
# Phase 3: colocated Z5, Z1, Z4
run_step colocated_provision_2 provision_colocated
run_step Z5_colocated bash "$ROOT/scripts/run-scylla-track-z.sh" native-colocated z5 256
run_step Z1_colocated bash "$ROOT/scripts/run-scylla-track-z.sh" native-colocated z1 256
run_step Z4_colocated bash "$ROOT/scripts/run-scylla-track-z.sh" native-colocated z4 256
run_step fetch_colocated_2 bash "$ROOT/scripts/fetch-reports.sh" native-colocated --upload-s3 || true
bash "$ROOT/scripts/upload-diagnosis-state.sh" || true

run_step teardown_all bash "$ROOT/scripts/teardown-all.sh"

trap - EXIT
echo "SCYLLA_LEVERS_COMPLETE"
