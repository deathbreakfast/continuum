#!/usr/bin/env bash
# Master driver: Track AA index scaling matrix (1n/2n/4n t3, WSL-resilient).
# Usage: run-scylla-index-scaling-master.sh [--no-teardown-on-fail] [--skip-artifact] [--from-step STEP]
set -euo pipefail
...
STATE_FILE="$STATE_DIR/scylla-index-scaling.json"
...
run_index_phase() {
  ...
  run_step "${prefix}_index_scaling" bash "$ROOT/scripts/run-scylla-index-scaling.sh" "$topo"
...
run_index_phase 1n native-scylla-1n
run_index_phase 2n native-scylla-2n
run_index_phase 4n native-scylla-4n

# Track AB: raw baseline on 4n (July run failed)
run_step track_w_4n_provision provision_topo native-scylla-4n
run_step track_w_4n bash "$ROOT/scripts/run-scylla-track-w.sh" native-scylla-4n
run_step track_w_4n_fetch bash "$ROOT/scripts/fetch-reports.sh" native-scylla-4n --upload-s3 || true
run_step track_w_4n_teardown bash "$ROOT/scripts/teardown.sh" native-scylla-4n

run_step teardown_all bash "$ROOT/scripts/teardown-all.sh"

trap - EXIT
echo "SCYLLA_INDEX_SCALING_COMPLETE"
